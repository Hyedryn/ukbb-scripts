...
datalad create ${DATASET_TMPDIR}/sub-${PARTICIPANT_ID} --force
cd ${DATASET_TMPDIR}/sub-${PARTICIPANT_ID}
datalad ukb-init --bids --force ${PARTICIPANT_ID} 20227_2_0 20252_2_0
datalad ukb-update --merge --force --keyfile "./"
cd ${SLURM_TMPDIR}
deactivate

# fmriprep job
singularity run --cleanenv -B ${DATASET_TMPDIR}:/WORK -B ${HOME}/.cache/templateflow:/templateflow -B /etc/pki:/etc/pki/ \
...
# save outputs
rsync -rltv --info=progress ${DATASET_TMPDIR}/fmriprep_work ${OUTPUT_DIR}/fmriprep_sub-${PARTICIPANT_ID}.workdir
if [ $fmriprep_exitcode -eq 0 ] ; then
    rsync -rltv --info=progress ${DATASET_TMPDIR}/derivatives/fmriprep/* ${OUTPUT_DIR}
